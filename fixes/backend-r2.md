...
| F-05 | media | Spec "post the handover link in Slack" | El `channel.send` lo redacta el modelo **antes** de ejecutar, así que no puede contener el link del documento que se crea recién al aprobar. | El writer devuelve `documentId` y `title`; `get_document` **no trae URL** y la app responde 200 a cualquier ruta, así que no se puede deducir el patrón. **Ivan: copiar del navegador la URL de un doc** (ej. `Handover Tarde 2026-09-11`) y pegarla acá; con eso el writer arma `url` y la card la muestra. Mientras tanto la card muestra id y título. | R4 + R2 | ⏳ |
...
| 14:02 | `documents_delete` + `documents_trash_list` + `list_documents` | Doc de prueba en papelera; los 4 docs sembrados intactos | Limpieza verificada. El workspace demo quedó como estaba. |
